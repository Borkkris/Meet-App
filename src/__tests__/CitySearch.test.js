...
describe('<CitySearch /> component', () => {
    ...
    test('suggestion list match the query when changed', () => {
        CitySearchWrapper.setState({ query: '', suggestions: [] }); // states for both have been emptied for the next step
        CitySearchWrapper.find('.city').simulate('change', { 
            target: { value: "Berlin" } // changing the target to "Berlin" hrough a simulated change event on the .city input field
        });
        const query = CitySearchWrapper.state('query');
