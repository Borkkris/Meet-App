...
const credentials = {
  ...
  redirect_uris: ['https://borkkris.github.io/meet/'],
  javascript_origins: ['https://borkkris.github.io', 'http://localhost:3000'],
};
...
module.exports.getCalendarEvents = async (event) => {
  ...
  return new Promise( (resolve, reject) => {
    ...
    // list is a method with two arguments in it
    calendar.events.list(
      // return value of the list method:
      {
      ...
      (error, response) => {
        if (error) {
          reject(error);
        } else { // product of the return value of the  list method up there
          resolve(response); // if the calendar.events.list happens successfully its gonna pass the data to this resolve object (response) and then goes down to the .then statement.
        }
      }
    );
  })
        // this results variable right here is actually the argument from the resolve call up there / the data from the googles calendar app
  .then((results) => {
    return{
      statusCode: 200,
      headers: {
      ...
      body: JSON.stringify({ events: results.data.items }) // shown at the front end
    };
  })
  // gets fired when an error occures
  .catch( error => {
    console.error(error)
      // Handle error
      return {
